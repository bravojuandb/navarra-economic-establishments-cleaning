...
Note: Raw and processed datasets are generated by the pipeline and are not committed to the repository.
...
→ handle_nulls
→ fix_string_numerals
→ cast_year_to_int (type enforcement)
→ dedupe_rows (deduplication of identical rows)
